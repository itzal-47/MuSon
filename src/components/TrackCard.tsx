...
import { Play, Pause, Music, BadgeCheck, Heart, ListPlus } from 'lucide-react';
...
export default function TrackCard({ track, queue, variant = 'list' }: Props) {
  ...
  const handleAddToPlaylist = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!user) {
      requireLogin('Inicia sessão para criar e gerir playlists.');
      return;
    }
  ...
  if (variant === 'card') {
    return (
      <>
        ...
        >
          <div className={`relative w-40 h-40 rounded-2xl overflow-hidden bg-neutral-900 border ${isCurrent ? 'border-amber-600 glow-accent-sm' : 'border-neutral-800'}`}>
            ...
            <button
              ...
              className="absolute bottom-2 left-2 w-8 h-8 rounded-full bg-black/60 backdrop-blur flex items-center justify-center"
            >
        ...
        {showAddToPlaylist && (
          <AddToPlaylistModal trackId={track.id} onClose={() => setShowAddToPlaylist(false)} />
        )}
      </>
    );
  }
  ...
  return (
    <>
      ...
      >
        ...
        <button onClick={handleAddToPlaylist} className="shrink-0 p-2 text-neutral-500 hover:text-amber-500 transition-colors">
          <ListPlus size={18} />
        </button>
